      ...
      {catalog.map((item) => (
        <div className="grid gap-2 border-2 p-2" key={item.id}>
          ...
          <div className="grid gap-1.5">
            ...
            <div className="">
              Por<span className="font-semibold"> {item.user[0].name}</span>{" "}
            </div>
          ...
          <button className="bg-green-500 rounded h-10 text-white hover:bg-green-400">
            <ModalCatalogo id={item.id} user_id={item.user_id} />
          </button>
        </div>
      ))}
    </div>
